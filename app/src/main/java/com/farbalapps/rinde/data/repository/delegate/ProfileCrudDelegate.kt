...
class ProfileCrudDelegate @Inject constructor(
    ...
    suspend fun syncProfile(userId: String) {
        ...
        val profile = if (data != null) {
            parseExistingProfile(userId, data)
        } else {
    ...
    private suspend fun parseExistingProfile(userId: String, data: MutableMap<String, Any>): Profile {
        ...
        if (needsPatch) {
            applyProfilePatch(userId, data, resolvedName, resolvedPhotoUrl)
        }

        return buildProfileFromData(userId, resolvedName, resolvedPhotoUrl, data)
    }

    private suspend fun applyProfilePatch(userId: String, data: MutableMap<String, Any>, resolvedName: String, resolvedPhotoUrl: String?) {
    ...
    private fun buildProfileFromData(userId: String, resolvedName: String, resolvedPhotoUrl: String?, data: Map<String, Any>): Profile {
    ...
    private suspend fun createInitialProfile(userId: String): Profile {
        ...
        return Profile(id = userId, name = newName, photoUrl = firebaseAuthPhotoUrl, isDummy = false)
    }
    ...
    suspend fun updateProfile(userId: String, name: String, photoUrl: String?): Result<Unit> = runCatching {
        ...
        val localOptimizedFilePath = optimizePhotoUrl(photoUrl)
        val targetPhotoUrl = localOptimizedFilePath ?: photoUrl

        updateLocalProfile(userId, validatedName, targetPhotoUrl)
        launchProfileUpdateProcess(userId, validatedName, photoUrl, localOptimizedFilePath, targetPhotoUrl)
    }

    private fun optimizePhotoUrl(photoUrl: String?): String? {
        if (photoUrl != null && (photoUrl.startsWith("content://") || photoUrl.startsWith("file://"))) {
            ...
            return optimizedFile?.absolutePath
        }
        return null
    }

    private suspend fun updateLocalProfile(userId: String, name: String, targetPhotoUrl: String?) {
    ...
    private fun launchProfileUpdateProcess(userId: String, validatedName: String, originalPhotoUrl: String?, localOptimizedFilePath: String?, targetPhotoUrl: String?) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val finalPhotoUrl = processPhotoUpload(userId, originalPhotoUrl, localOptimizedFilePath, targetPhotoUrl)
                updateFirestoreAndAuth(userId, validatedName, finalPhotoUrl)
                propagateProfileChangesToPosts(userId, validatedName, finalPhotoUrl)
                syncProfile(userId)
    ...
    private suspend fun processPhotoUpload(userId: String, originalPhotoUrl: String?, localOptimizedFilePath: String?, targetPhotoUrl: String?): String? {
        var finalPhotoUrl: String? = targetPhotoUrl
        val isDeletion = originalPhotoUrl == null && localOptimizedFilePath == null
    ...
    private suspend fun updateFirestoreAndAuth(userId: String, validatedName: String, finalPhotoUrl: String?) {
