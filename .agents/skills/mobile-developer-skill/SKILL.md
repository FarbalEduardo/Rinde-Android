...
15. **Aislamiento de Modelos de Datos**: Detecta si modelos de la capa `data` o anotaciones específicas de frameworks de datos (`@Entity`, `@SerializedName`, `@Json`, etc.) se filtran hacia las capas de `domain` o `presentation`.
...
21. **Denormalization Contract Checker**: Asegurar que al modificar entidades primarias (como `Profile`), el repositorio propague adecuadamente los cambios a las entidades duplicadas en batches (`posts`, `saved_posts`), siguiendo la skill transversal `firestore-denormalization-skill`.
